...
> die Adresse aus `npm run adresse`. Wen es doch stört: [eine feste Adresse](#eine-feste-adresse-der-eigene-vorposten)
> kostet nichts extra – nur die Domain.
...
### Eine feste Adresse: der eigene Vorposten

Wem das Weitersagen zu bunt wird, gibt seiner Runde eine Adresse, die nie mehr wechselt – die eigene.

Das Hindernis: Ein Laptop hat keine feste Adresse im Netz, eine Domain kann also nicht auf ihn zeigen.
Der Ausweg ist ein **Vorposten** – ein kleiner Server, der immer am selben Fleck steht. Auf ihn zeigt
die Domain, dort liegt das Zertifikat, und weil der Laptop hinausrufen darf, auch wenn niemand
hineinrufen kann, dreht die Leitung die Richtung um:
...
Das Skript holt nginx und certbot, legt einen Zugang an, der ausschließlich diese eine Rückleitung
aufmachen darf, stellt den Live-Kanal puffungsfrei durch und besorgt das Zertifikat.

Danach zwei Zeilen in der `.env` auf dem Laptop:
...
npm run tunnel   # die Leitung zum Vorposten (baut sich nach Abbrüchen selbst wieder auf)
```

Danach nennt der Almanach die Adresse schon beim Start, und `npm run adresse` sucht gar nicht mehr
nach geliehenen – im Protokoll läge sonst womöglich noch eine von vorgestern, und die führte die
Runde ins Leere. Schritt für Schritt, samt der beiden Oracle-Stolperstellen, steht das im
[Einrichtungs-Handbuch 6.5](docs/EINRICHTUNG.md#65-die-feste-adresse-eigene-domain-über-einen-vorposten).
...
> Anschlüssen gibt der Anbieter gar keine eigene öffentliche Adresse mehr heraus. Der Vorposten
> umgeht das alles – und nimmt den Umzug gleich mit: Die Leitung trägt aus jedem WLAN.

Auf dem Vorposten liegt übrigens nichts von euch. Charaktere, Karten, Chronik und Bildnisse bleiben
auf dem Laptop; dort stehen nur nginx und das Zertifikat. Geht er verloren, ist er in zehn Minuten
neu angelegt.
...
| `TUNNEL_ZIEL`    | leer                               | Der Vorposten, bei dem sich dieser Rechner meldet (`tunnel@www.deinemudda.fun`) |
| `TUNNEL_FERNPORT`| wie `PORT`                         | Port, an dem der Almanach auf dem Vorposten hängt – nur nötig, wenn 3001 dort belegt ist |
...
scripts/    Hilfsskripte, plattformunabhängig in Node geschrieben
            ...
            tunnel.mjs       den Weg nach außen ohne Docker aufmachen –
                             Schnelltunnel, mit `TUNNEL_ZIEL` zum eigenen Vorposten
...
| `npm run tunnel`    | Den Weg von außen aufmachen, ohne Docker (`Strg+C` schließt ihn); mit `TUNNEL_ZIEL` die Leitung zum eigenen Vorposten |
